...
from models import (
  User, Attendance, RemovedEmployee, UnknownRFID, Room, Department,
  Task, LeaveRequest, Team, Project, ProjectTask, ProjectAssignment,
  ProjectTaskAssignee, AttendanceLog, AttendanceDaily, Payroll, Performance
)
...
@app.post("/admin/performance/add")
async def add_performance(
    employee_id: str = Form(...),
    rating: float = Form(...),
    communication: float = Form(...),
    teamwork: float = Form(...),
    productivity: float = Form(...),
    punctuality: float = Form(...),
...
async def performance_dashboard(
    ...
    records = db.query(Performance).order_by(Performance.created_at.desc()).all()

    return templates.TemplateResponse(
        "admin/admin_performance.html",
