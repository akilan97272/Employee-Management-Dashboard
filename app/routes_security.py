...
EVENT_INGEST_INTERVAL_SEC = max(2, int(os.getenv("SECURITY_EVENT_INGEST_INTERVAL_SEC", "5")))
_event_ingest_last_ts = 0.0
_event_ingest_lock = Lock()
...
def _ensure_default_configuration_settings(db: Session) -> None:
    ...
    data_storage_mode = str(os.getenv("DATA_STORAGE_MODE", "both")).strip().lower()
    if data_storage_mode not in {"regular", "hash_only", "both"}:
        data_storage_mode = "both"
    activity_logging_enabled = "true" if _as_bool(os.getenv("ACTIVITY_LOGGING_ENABLED"), True) else "false"
    activity_toggle_enabled = "true" if _as_bool(os.getenv("ACTIVITY_LOGGING_ENABLED"), True) else "false"

    secret_key = str(os.getenv("SECRET_KEY") or os.getenv("SESSION_SECRET_KEY") or "").strip()
    if not secret_key:
        secret_key = secrets.token_urlsafe(64)
    session_secret_key = str(os.getenv("SESSION_SECRET_KEY") or secret_key).strip()

    encryption_key = str(os.getenv("ENCRYPTION_KEY") or os.getenv("DATA_ENCRYPTION_KEY") or "").strip()
    if not encryption_key:
        encryption_key = base64.urlsafe_b64encode(secrets.token_bytes(32)).decode("utf-8")
    data_encryption_key = str(os.getenv("DATA_ENCRYPTION_KEY") or encryption_key).strip()

    os.environ["SESSION_MAX_AGE"] = session_max_age
    os.environ["SESSION_IDLE_TIMEOUT"] = session_idle_timeout
    os.environ["DATA_STORAGE_MODE"] = data_storage_mode
    os.environ["ACTIVITY_LOGGING_ENABLED"] = activity_logging_enabled
    ...
    defaults: list[tuple[str, str, str]] = [
        ("session-security", "SESSION_MAX_AGE", session_max_age),
        ("session-security", "SESSION_IDLE_TIMEOUT", session_idle_timeout),
        ("data-integrity", "DATA_STORAGE_MODE", data_storage_mode),
        ("activity-logging", "ACTIVITY_LOGGING_ENABLED", activity_logging_enabled),
        ("activity-logging", "__enabled__", activity_toggle_enabled),
        ("security-config", "DATA_ENCRYPTION_KEY", data_encryption_key),
        ("security-config", "ENCRYPTION_KEY", encryption_key),
        ("security-config", "SESSION_SECRET_KEY", session_secret_key),
        ("security-config", "SECRET_KEY", secret_key),
    ]

    changed = False
    for feature_id, key, default_value in defaults:
        row = (
            db.query(SecurityManagedSetting)
            .filter(SecurityManagedSetting.feature_id == feature_id, SecurityManagedSetting.key == key)
            .first()
        ...
        if row:
            continue
        db.add(SecurityManagedSetting(feature_id=feature_id, key=key, value=default_value))
        changed = True
    if changed:
        db.commit()
...
def _ingest_security_events_to_db(db: Session) -> None:
    global _event_ingest_last_ts
    now_ts = time.time()
    if (now_ts - _event_ingest_last_ts) < EVENT_INGEST_INTERVAL_SEC:
        return
    if not _event_ingest_lock.acquire(blocking=False):
        return
    _ensure_security_runtime_schema(db)
    try:
        now_ts = time.time()
        ...
        existing = {
            fp
            for (fp,) in (
                db.query(SecurityEventRecord.fingerprint)
        ...
        for path, etype in (("logs/security.log", "request"), ("logs/audit.log", "audit")):
            ...
            lines = [line.strip() for line in blob.splitlines()[-1500:] if line.strip()]
            for line in lines:
                fingerprint = hashlib.sha256(f"{etype}|{line}".encode("utf-8", errors="ignore")).hexdigest()
        ...
        if added:
            db.commit()
        _event_ingest_last_ts = time.time()
    finally:
        _event_ingest_lock.release()
...
def _security_live_payload(db: Session) -> dict:
    ...
    events = _recent_security_events(db, limit=300)
...
def _security_live_signature(payload: dict) -> str:
    ...
    signature_obj = {
        ...
        "latest_configuration": payload.get("latest_configuration", {}) if isinstance(payload.get("latest_configuration", {}), dict) else {},
        "latest_mitre_rule": payload.get("latest_mitre_rule", {}) if isinstance(payload.get("latest_mitre_rule", {}), dict) else {},
    }
...
def _security_dashboard_payload(db: Session) -> dict:
    ...
    latest_mitre_rule = (
        db.query(SecurityMitreRule)
        .order_by(SecurityMitreRule.updated_at.desc(), SecurityMitreRule.id.desc())
        .first()
    )
    payload["latest_configuration"] = {
        "id": int(latest_config.id),
        "feature_id": latest_config.feature_id,
        "key": latest_config.key,
        "updated_at": latest_config.updated_at.isoformat() if latest_config and latest_config.updated_at else "",
    } if latest_config else {}
    payload["latest_mitre_rule"] = {
        "id": int(latest_mitre_rule.id),
        "name": latest_mitre_rule.name,
        "enabled": bool(latest_mitre_rule.enabled),
        "updated_at": latest_mitre_rule.updated_at.isoformat() if latest_mitre_rule and latest_mitre_rule.updated_at else "",
    } if latest_mitre_rule else {}
...
async def admin_security_page(request: Request, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    _admin_guard(user)
    _ensure_default_configuration_settings(db)
    all_features = _security_features(db)
    features = _dashboard_features(all_features)
    events = _recent_security_events(db, limit=600)
    hash_history = read_hash_history(limit=1500)
