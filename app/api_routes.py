...
def register_api_routes(app):
    @app.api_route("/api/attendance", methods=["GET", "POST"])
    async def record_attendance(
        ...
        GATE_ROOM_NO = "77"
        incoming_employee_id = _pick(employee_id, payload.get("employee_id"), payload.get("emp_id"))
        incoming_rfid_tag = _pick(rfid_tag, payload.get("rfid_tag"), payload.get("rfid"), payload.get("tag_uid"), payload.get("uid"))

        provided_area_id = _pick(area_id, payload.get("area_id"))
        provided_building_id = _pick(building_id, payload.get("building_id"), payload.get("id"))
        provided_floor_no = _normalize_floor(_pick(floor_no, payload.get("floor_no"), payload.get("floor"), payload.get("level")))

        provided_area = _pick(area, area_name, payload.get("area"), payload.get("area_name"), payload.get("location_name"), payload.get("location"))
        provided_building = _pick(
            building_name,
            payload.get("building_name"),
            building_no,
            payload.get("building_no"),
            payload.get("slot_no"),
            payload.get("room_no"),
            payload.get("block"),
        )

        if provided_area_id and not provided_area:
            try:
                area_row = db.query(Area).filter(Area.id == int(provided_area_id)).first()
                if area_row:
                    provided_area = _clean(area_row.name)
            except Exception:
                pass

        if provided_building_id:
            room_id_lookup = provided_building_id if str(provided_building_id).upper().startswith("R") else f"R{provided_building_id}"
            room_by_manual_id = db.query(Room).filter(func.lower(Room.room_id) == room_id_lookup.lower()).first()
            if room_by_manual_id:
                if not provided_area:
                    provided_area = _clean(room_by_manual_id.location_name)
                if not provided_building:
                    provided_building = _clean(room_by_manual_id.room_no)
                # Normalize to mapped room fields for downstream validation and attendance write.
                location_name = _clean(room_by_manual_id.location_name)
                room_no = _clean(room_by_manual_id.room_no)
            try:
                building_row = db.query(Building).filter(Building.id == int(provided_building_id)).first()
                if building_row:
                    if not provided_building:
                        provided_building = _clean(building_row.name)
                    if not provided_area:
                        area_row = db.query(Area).filter(Area.id == building_row.area_id).first()
                        if area_row:
                            provided_area = _clean(area_row.name)
                    if provided_area_id and str(building_row.area_id) != str(provided_area_id):
                        return {
                            "status": "invalid_area_building",
                            "error": "building_id does not belong to the provided area_id",
                        }
            except Exception:
                pass

        resolved_location_name = _pick(provided_area, location_name, payload.get("area"), payload.get("area_name"), payload.get("location_name"), payload.get("location"))
        resolved_room_no = _pick(slot_no, provided_building, room_no, payload.get("slot_no"), payload.get("building_name"), payload.get("building_no"), payload.get("room_no"), payload.get("block"))
        resolved_floor_no = _normalize_floor(_pick(provided_floor_no, payload.get("floor_no"), payload.get("floor"), payload.get("level")))

        if not resolved_location_name:
            resolved_location_name = "Main Gate" if resolved_room_no == GATE_ROOM_NO else ""
        if not resolved_room_no:
            raise HTTPException(status_code=400, detail="room_no or slot_no/building_name is required")
        if not resolved_location_name:
            raise HTTPException(status_code=400, detail="location_name or area is required")

        if incoming_rfid_tag:
            user = db.query(User).filter(User.rfid_tag == incoming_rfid_tag, User.is_active == True).first()
        elif incoming_employee_id:
            user = db.query(User).filter(User.employee_id == incoming_employee_id, User.is_active == True).first()
        else:
            raise HTTPException(status_code=400, detail="rfid_tag or employee_id is required")

        if not user:
            db.add(UnknownRFID(rfid_tag=(incoming_rfid_tag or incoming_employee_id), location=resolved_location_name))
            db.commit()
            return {"status": "unknown_rfid"}

        # Strict infra rule: when building_name (or legacy building_no) + area are provided, mapping must be valid.
        if provided_area and provided_building:
            mapped_pair = (
                db.query(Building.id)
                .join(Area, Area.id == Building.area_id)
                .filter(
                    func.lower(Area.name) == provided_area.lower(),
                    func.lower(Building.name) == provided_building.lower(),
                )
                .first()
            )
            if not mapped_pair:
                db.add(InappropriateEntry(
                    ...
                    rfid_tag=(incoming_rfid_tag or user.rfid_tag or incoming_employee_id),
                    location_name=resolved_location_name,
                    room_no=resolved_room_no,
                    reason=f"Building '{provided_building}' is not mapped under area '{provided_area}'"
                ))
                db.commit()
                return {
                    "status": "invalid_area_building",
                    "error": "building_name is not mapped to the provided area",
                }

        # Validate that room exists in Room table (only if not gate room)
        if resolved_room_no != GATE_ROOM_NO:
            ...
            if not valid_room:
                ...
                db.add(InappropriateEntry(
                    ...
                    location_name=resolved_location_name,
                    room_no=resolved_room_no,
                    reason=f"Room '{resolved_room_no}' in '{resolved_location_name}' not found in Room table"
                ))
                db.commit()
        ...
        new_log = AttendanceLog(
            ...
            location_name=resolved_location_name,
            room_no=resolved_room_no,
            floor_no=resolved_floor_no or None,
        )
        db.add(new_log)
        ...
        if resolved_room_no == GATE_ROOM_NO:
            ...
            if open_gate:
                ...
                db.add(Attendance(employee_id=user.employee_id, date=today, entry_time=now, status="PRESENT", location_name=resolved_location_name, room_no=GATE_ROOM_NO, floor_no=None))
                status_msg = "gate_entered"
        else:
            if not open_gate:
                db.add(Attendance(employee_id=user.employee_id, date=today, entry_time=now, status="PRESENT", location_name="Main Gate", room_no=GATE_ROOM_NO, floor_no=None))

            open_floor = _normalize_floor(getattr(open_block, "floor_no", "")) if open_block else ""
            same_room_and_floor = bool(open_block and open_block.room_no == resolved_room_no and open_floor == resolved_floor_no)
